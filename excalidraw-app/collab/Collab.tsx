...
class Collab extends PureComponent<Props, CollabState> {
  ...
  private beforeUnload = withBatchedUpdates((event: BeforeUnloadEvent) => {
    ...
    if (
      ...
      (this.fileManager.shouldPreventUnload(syncableElements) ||
        !isPersisted(this.portal, syncableElements))
    ) {
  ...
  saveCollabRoomToFirebase = async (
    ...
    try {
      const savedData = await saveToPostgres(
        this.portal,
        syncableElements,
  ...
  startCollaboration = async (
    ...
    try {
      ...
      console.log("The url is:", socketServerData.url);
  ...
  private initializeRoom = async ({
    ...
    if (fetchScene && roomLinkData && this.portal.socket) {
      ...
      try {
        const elements = await loadFromPostgres(
          roomLinkData.roomId,
          this.portal.socket,
        );
        if (elements) {
